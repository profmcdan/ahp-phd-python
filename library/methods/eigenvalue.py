...
from library.methods import Method
...
class EigenvalueMethod(Method):
    ...
    def estimate(self, preference_matrix):
        ...
        _, vectors = eigs(preference_matrix, k=(width-2),
                          sigma=width, which='LM', v0=np.ones(width))

        real_vector = np.real([vec for vec in np.transpose(
            vectors) if not np.all(np.imag(vec))][:1])
